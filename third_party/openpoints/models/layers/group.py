...
class KNNGroup(nn.Module):
    def __init__(self, nsample: int, relative_xyz=True, normalize_dp=False, return_only_idx=False, **kwargs):
        """[summary]
...
class Grouping(Function):
    ...
    def forward(ctx, features, indices):
        ...
        return pointnet2_cuda.grouping_forward(features, indices)

    @staticmethod
    def backward(ctx, grad_output):
        (indices,) = ctx.saved_tensors
        grad_features = pointnet2_cuda.grouping_backward(grad_output.contiguous(), indices, ctx.num_points)
        return grad_features, None


pvcnn_grouping = Grouping.apply
...
if __name__ == "__main__":
    ...
    # --------------- debug downsampling
    from openpoints.models.layers.layer3d import furthest_point_sample, random_sample
