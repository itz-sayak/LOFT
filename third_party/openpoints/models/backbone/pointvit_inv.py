...
class InvPointViT(nn.Module):
        ...
        if self.option != -1:
            ...
            if epoch < self.init_epoch:  # Inference
                    ...
                    alpha = round(
                        min(
                            max(
                                ...
                                * math.ceil((epoch - start_epoch - 1 - self.freq // 2) / self.freq)
                                + 1,
                                0,
                            ),
                            1.0,
                ...
                # self.option 1: linear alpha = (1/(A+1-N))*x + N/(N-A-1), lambd = 1 - alpha + 0.01
                    ...
                    alpha = round(
                        1 / (start_epoch + 1 - end_epoch) * epoch + end_epoch / (end_epoch - start_epoch - 1),
                        2,
                    )
            ...
            if epoch < self.init_epoch:  # Inference
                ...
                if self.option == 6:
                    lambd = -1 * round(
                        max(
                            min(
                                self.freq / (end_epoch - start_epoch) * math.ceil((epoch - start_epoch - 1) / self.freq)
                                + self.init_lambd,
                                1.0,
                            ),
                            self.init_lambd,
    ...
    def lambd_alpha_options(self, option=0, current=-1, start=0, end=10, freq=10.0, freeze_ratio=0.6):
        """
        ...
        if option == 0:
            lambd = round(
                max(
                    min(
                        (freq / (end - start) * math.ceil((current - start - 1) / freq) + lambd_min),
                        lambd_max,
                    ),
                    lambd_min,
                ),
            ...
            alpha = round(
                min(
                    max(
                        -round(freq / (end - start), 12) * math.ceil((current - start - 1 - freq // 2) / freq) + 1,
                        alpha_min,
                    ),
                    alpha_max,
                ),
        ...
        #     alpha = 1.0
            ...
            if current < end_half:
                ...
                lambd = round(
                    max(
                        min(
                            freq / (end - end_half) * math.ceil((current - end_half - 1) / freq) + lambd_min,
                            lambd_max,
                        ),
                        lambd_min,
                    ),
            ...
            alpha = round(
                min(
                    max(
                        -round(freq / (end_half - start), 12) * math.ceil((current - start - 1 - freq // 2) / freq) + 1,
                        alpha_min,
                    ),
                    alpha_max,
                ),
            ...
            lambd = round(
                max(
                    min(
                        (freq / (end_half - start) * math.ceil((current - start - 1) / freq) + lambd_min),
                        lambd_max,
                    ),
                    lambd_min,
                ),
            ...
            if current < end_half:
                ...
                alpha = round(
                    min(
                        max(
                            -round(freq / (end - end_half), 12) * math.ceil((current - end_half - 1 - freq // 2) / freq)
                            + 1,
                            alpha_min,
                        ),
                        alpha_max,
...
class InvPointViTPartDecoder(nn.Module):
        ...
        **kwargs,
        ...
        if self.cls_map == "curvenet":
            ...
            self.global_conv2 = nn.Sequential(
                create_convblock1d(fp_channels[-1] * 2, 128, norm_args=None, act_args=act_args)
            )
            ...
            self.convc = nn.Sequential(create_convblock1d(16, 64, norm_args=None, act_args=act_args))
    ...
    def forward(self, p, f, cls_label):
        ...
        f[i - 1] = self.decoder[i][1:](
            [
                p[i],
                self.decoder[i][0]([p[i - 1], torch.cat([cls_one_hot, f[i - 1]], 1)], [p[i], f[i]]),
            ]
        )[1]
